    ...
    switch (m_state) {
        ...
        case STATE_REAL_APPROACH: {
            ...
            if (m_approach_ctr > ACCEL_LOOPS && wide_bar_active()) {
                ...
                m_color_pending    = 0;
                m_arm_retract_ctr  = 0;
                // Sofort Tray ausrichten wenn Farbe schon aus Anfahrt bekannt
        ...
        case STATE_REAL_FOLLOW: {
            ...
            if (m_guard_ctr > 0) {
                ...
                m_color_pending    = 0;
                m_arm_retract_ctr  = 0;
                // Sofort Tray ausrichten wenn Farbe schon bekannt
                if (m_color_fallback != 0) {
                    m_action_color = m_color_fallback;
                    int slot = colorToSlot(m_action_color);
                    if (slot >= 0) {
        ...
        case STATE_CROSSING_STOP: {
            ...
            if (m_crossing_ctr > CROSSING_STOP_LOOPS - COLOR_READ_PHASE) {
                ...
                if (m_color_stable_ctr >= COLOR_STOP_STABLE_CNT && m_action_color == 0) {
                    ...
                    // Sofort Tray ausrichten sobald Farbe bestätigt
                    int slot = colorToSlot(m_action_color);
                    if (slot >= 0) {
                        m_target_angle = COLOR_ANGLE[slot];
                        trayMoveTo(m_target_angle);
            ...
            // Ende Farblese-Phase: Farbe fixieren + Arm starten
            if (m_crossing_ctr == CROSSING_STOP_LOOPS - COLOR_READ_PHASE) {
                if (m_action_color == 0) {
                    m_action_color = m_color_fallback;
                    // Tray nur starten wenn noch nicht gestartet
                    int slot = colorToSlot(m_action_color);
                    if (slot >= 0) {
                        m_target_angle = COLOR_ANGLE[slot];
                        trayMoveTo(m_target_angle);
