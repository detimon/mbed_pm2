...
static const int   SMALL_FOLLOW_START_GUARD  = static_cast<int>(781 * SPEED_SCALE); // [SPEED_SCALE] 15.62 s @ MAX_SPEED=1.0 — ignoriert b3/b4/b5 nach 4. Querbalken (+1.5s vs v20)
...
static const float SERVO_D2_BLAU_DOWN     = 0.43f;   // D2 Tiefe 0.37f für BLAU (ca. 4 mm weniger tief als 0.34)
static const float SERVO_D2_GRUEN_DOWN    = 0.27f;   // D2 Tiefe 0.16f für GRÜN (ca. 4 mm weniger tief als 0.13)
    ...
    switch (m_state) {
        ...
        case STATE_CROSSING_STOP:
            ...
            if ((m_action_color == 5 || m_action_color == 7) && m_arm_retract_ctr > 0) {
                ...
                if (rev && m_arm_retract_ctr == 140) g_servo->enable(SERVO360_CORRECT_SPEED); // zurück zu 45°
                if (rev && m_arm_retract_ctr == 137) g_servo->enable(0.5f);   // stop
                if (rev && m_arm_retract_ctr == 112) {                         // D2 voll runter
                ...
                if (m_arm_retract_ctr == 52) g_servo->enable(rev ? 0.52f : 0.37f); // Jiggle Richtung 2a (1. Impuls)
                if (m_arm_retract_ctr == 46) g_servo->enable(0.5f);               // stop — 6 Loops Richtung 2a
                if (m_arm_retract_ctr == 43) g_servo->enable(rev ? 0.52f : 0.37f); // Jiggle Richtung 2b (2. Impuls, doppelt)
                if (m_arm_retract_ctr == 37) {
                    g_servo->enable(0.5f);                                         // stop — 6 Loops Richtung 2b
                ...
                if (m_arm_retract_ctr == 0) {
                    g_servo_D1->disable();
                    // Alle Balken (inkl. letzter): 5 Clicks → Tablett weiterschalten
...
/*
        ...
        // Balken 1+Schmallinien: 138 Loops; Balken 2/3/4: 180 Loops (inkl. 0.5s+7-Loop-Korrektur+0.5s)
        // ----------------------------------------------------------------
        case STATE_ROT_GELB_PAUSE: {
            ...
            if (m_arm_retract_ctr > 0) {
                ...
                if (m_arm_retract_ctr == 165 &&  rev) g_servo_D2->enable(SERVO_D2_PARTIAL_DOWN); // ~3cm runter
                if (m_arm_retract_ctr == 138 && !rev) g_servo_D1->enable(0.95f);
                if (m_arm_retract_ctr == 123 && !rev) g_servo_D2->enable(d2_depth);
                if (rev && m_arm_retract_ctr == 140) g_servo->enable(SERVO360_CORRECT_SPEED); // zurück zu 45°
                if (rev && m_arm_retract_ctr == 137) g_servo->enable(0.5f);   // stop
                if (rev && m_arm_retract_ctr == 112) g_servo_D2->setPulseWidth(d2_depth); // D2 voll runter
                // Alle Balken: Jiggle-Richtung abhängig ob 5-Click voranging (rev) — bei ROT an breitem Balken umgekehrt, bei ROT an Schmallinie zurück zur Standard-Richtung
                const bool jiggle_rev = rev ^ (m_rot_gelb_color == 3 && !m_rot_gelb_is_small);
                if (m_arm_retract_ctr == 108) g_servo->enable(jiggle_rev ? 0.37f : 0.52f); // Jiggle Richtung 1
                if (m_arm_retract_ctr == 101) g_servo->enable(0.5f);          // stop — 7 Loops Richtung 1
                ...
                if (m_arm_retract_ctr == 0) {
                    g_servo_D1->disable();
                    bool do_5click = !m_rot_gelb_is_small;
                    if (do_5click) {
                        // Alle breiten Balken (inkl. letzter): 5 Clicks → Tablett weiterschalten
                        g_servo->enable(SERVO360_CLICK_SPEED);
                        m_click_target = 5;
                        m_click_cnt    = 0;
                    } else {
                        // Schmallinien: 1 normale 90°-Drehung
                        g_servo->enable(SERVO360_KICK_SPEED);
                        m_click_target = 0;
                        m_click_cnt    = 0;
                    }
