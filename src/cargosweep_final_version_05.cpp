...
#ifdef CARGOSWEEP_FINAL_VERSION_05
...
// Tunnel: 3-Phasen-Sequenz (A: 2s Linefollower, B: 1.5s Gerade, C: Linefollower)
static const int TUNNEL_FOLLOW_LOOPS   = 100;  // 2s normaler Linefollower
static const int TUNNEL_STRAIGHT_LOOPS = 75;   // 1.5s geradeaus
...
static const int   CROSSING_STOP_LOOPS   = 250;  // war 150 — mehr Zeit für color reading bei wide bars
...
static const int   JIGGLE_LOOPS        = 65;  // +15 Loops = +0.3s längere Jiggle-Periode
static const int   RAISE_LOOPS         = 50;
static const int   RETRACT_LOOPS       = 120;
static const float JIGGLE_AMPL_DEG    = 10.5f;  // +1.5°
static const float D1_JIGGLE_OFFSET   = 0.09f;
static const int   D1_JIGGLE_PERIOD   = 30;    // Loops für EINE D1-Oszillation (0.6s)
static const int   TRAY_JIGGLE_PERIOD = 30;    // Loops pro Tray-Jiggle-Zyklus (0.6s)
static const float TRAY_JIGGLE_SPEED  = 0.35f; // muss > min_speed+Totzone sein damit Jiggle überwiegt
static const int   SF360_TIMEOUT_LOOPS = 150;  // war 60 — 3s für 270° GELB tray drehung
...
static const float SF360_MIN_SPEED   = 0.18f;  // über Totzone (~0.14) — sonst stallt Servo nahe Target
...
// Korrektur-Winkel = Basis-Zielwinkel - 35° (Anti-Slingshot während Annäherung).
// Tray fährt direkt hierher, dann Arm raus, dann beim D2-Absenken zurück auf base.
...
{
    // 360° Servo wird erst beim Ausrichten in STATE_BLIND aktiviert,
    ...
    switch (m_state) {
        ...
        case STATE_BLIND:
            ...
            if (all_sensors_active()) {
                ...
                //  Tray PWM ein → Karussell-Drehung beginnt via serviceTray()
        ...
        // ----------------------------------------------------------------
        // TUNNEL_FOLLOW: 3 Phasen
        //   A (0..TUNNEL_FOLLOW_LOOPS):        2s normaler Linefollower mit Anlauframpe
        //   B (..+TUNNEL_STRAIGHT_LOOPS):      1.5s geradeaus, kein Lenken
        //   C (danach):                        normaler Linefollower bis Schmallinie
        // ----------------------------------------------------------------
        case STATE_TUNNEL_FOLLOW: {
            ...
            if (m_small_accel_ctr <= TUNNEL_FOLLOW_LOOPS) {
                // Phase A: Linefollower mit Anlauframpe
                ...
                // Phase B: geradeaus, kein Linefollower
                g_cmd_M1 = VEL_SIGN * MAX_SPEED;
                g_cmd_M2 = VEL_SIGN * MAX_SPEED;
            } else {
                // Phase C: normaler Linefollower
                g_cmd_M1 = VEL_SIGN * g_lf->getLeftWheelVelocity();
                g_cmd_M2 = VEL_SIGN * g_lf->getRightWheelVelocity();
            }
            g_M1->setVelocity(g_cmd_M1);
        ...
        case STATE_YELLOW: {
            ...
            if (m_driving_back) {
                ...
                g_M1->setVelocity(0.0f);
                g_M2->setVelocity(0.0f);
                // Tray ist via pre-adjust schon zum Korrekturwinkel (base-35°) unterwegs.
                // Direkt warten bis er dort ankommt, dann Arm-Sequenz starten — smooth,
...
#endif // CARGOSWEEP_FINAL_VERSION_05
