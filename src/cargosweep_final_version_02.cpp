...
#ifdef CARGOSWEEP_FINAL_VERSION_02
...
static const float D1_EXTENDED        = 0.82f;
...
static const float JIGGLE_AMPL_DEG    = 7.0f;
...
static void serviceTray()
{
    ...
    if (in_arm_state) {
        // Always update first — reads fresh angle AND runs P-controller.
        ...
        // ccw_dist = how far the servo still has to spin CCW to reach target.
        // Safe window: ccw_dist <= 35° (at/just-past target) OR ccw_dist >= 185°
        // (already past halfway CW → P-controller shortest-path goes CCW).
        // Outside window (35°–185°): override P-controller with constant CCW.
        float ccw_dist = fmodf(g_servoTray->getCurrentAngle() - m_target_angle + 360.0f,
                               360.0f);
        bool in_window = (ccw_dist <= 35.0f || ccw_dist >= 185.0f);

        if (in_window) {
            // P-controller result already applied by update(); overlay jiggle.
            ...
            // Outside window: override with constant CCW until window opens.
            g_servoTray->stop();
...
#endif // CARGOSWEEP_FINAL_VERSION_02
