// CargoSweep — PROTOTYPE_02_V13
#include "test_config.h"

#ifdef PROTOTYPE_02_V13
...
void roboter_v13_task(DigitalOut& led)
{
    // --- Farbsensor ---
    ...
    *g_en = 1;
    ...
    // 360° Servo Ausrichtung: ISR-Flag → sofort stopp; oder nach 5s Fallback
    if (m_servo360_ctr > 0) {
        if (s_endstop_hit) {        // fallende Flanke von ISR gefangen → sofort stopp
            s_endstop_hit        = false;
            g_servo->disable();
            m_servo360_ctr       = 0;
            m_servo360_kick_ctr  = 0;
...
void roboter_v13_reset(DigitalOut& led)
{
    *g_en            = 0;
    g_M1->setVelocity(0.0f);
    g_M2->setVelocity(0.0f);
