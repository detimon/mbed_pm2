// CargoSweep — PROTOTYPE_02_V19
#include "test_config.h"

#ifdef PROTOTYPE_02_V19
...
static const float SERVO_D2_GRUEN_DOWN    = 0.13f;   // D2 Tiefe für GRÜN
static const float SERVO_D2_PARTIAL_DOWN  = 0.92f;   // D2 Zwischenposition (vor 360°-Korrektur, 1=oben)
...
static const int   SERVO360_SMALL_LOOPS   = 7;      // Timer für Balken 2/3/4: ~140ms fix-Drehung bis Endschalter
static const float SERVO360_CLICK_SPEED      = 0.58f;  // leicht über Kick-Speed für 5-Click
static const int   SERVO360_CLICK_EXTRA_LOOPS = 7;    // Extra-Loops nach 5. Click → Weiterdrehung
static const float SERVO360_CORRECT_SPEED    = 0.42f; // Korrektur zurück zu 45° nach 5-Click-Coast
...
enum State {
    ...
    STATE_ROT_GELB_DRIVE      = 16, // Linienfolge genau 120mm nach ROT/GELB
    STATE_ROT_GELB_PAUSE      = 17  // 1s Standstill danach
};
...
static Servo*        g_servo_D2 = nullptr;  // 180° servo B — D0 (PB_2)
static NeoPixel*     g_np       = nullptr;  // WS2812B colour-debug LED — PC_5 (A2)
static InterruptIn*      g_endstop      = nullptr;  // Endschalter — A0 (PC_2), PullUp, LOW=aktiv
static volatile bool     s_endstop_hit  = false;    // von ISR gesetzt bei fallender Flanke (1→0)

static State m_state          = STATE_BLIND;
static int   m_servo360_ctr   = 0;  // countdown loops für 360° Ausrichtung (0 = gestoppt)
static int   m_servo360_kick_ctr = 0; // kick-loops verbleibend (0 = schon auf Zielgeschwindigkeit)
static int   m_servo360_brake_ctr = 0; // brake delay after endstop hit (15 loops = 0.3s)
...
static int   m_click_cnt            = 0;     // gezählte Endschalter-Hits im 5-Click-Modus
...
void roboter_v19_init(int loops_per_second)
{
    ...
    static InterruptIn endstop(PC_2, PullUp);
    endstop.fall([]() { s_endstop_hit = true; }); // ISR: fallende Flanke = Schalter gedrückt
    ...
    g_cs->setCalibration();
    g_cs->switchLed(ON);
    g_servo->calibratePulseMinMax(0.0303f, 0.1223f); // kalibriert 2026-03-26 (test_drehservo_90)
...
void roboter_v19_task(DigitalOut& led)
{
    // --- Farbsensor ---
    ...
    *g_en = 1;
    ...
    if (m_servo360_ctr > 0) {
        if (s_endstop_hit && m_servo360_brake_ctr == 0 && m_servo360_kick_ctr == 0 && m_click_coast_ctr == 0) {
            ...
            if (stop_now) {
                if (m_click_target > 0 && SERVO360_CLICK_EXTRA_LOOPS > 0) {
                    // 5-Click-Modus: erst extra Loops weiterdrehen, dann bremsen
                    m_click_coast_ctr = SERVO360_CLICK_EXTRA_LOOPS;
                    m_click_cnt       = 0;
                    m_click_target    = 0;
                } else {
                    // Normaler Stop (Kick oder letzter Click ohne Extra)
                    g_servo->enable(0.5f);
            ...
            // else: continue spinning for next click
        }
        if (m_click_coast_ctr > 0) {
            m_click_coast_ctr--;
            if (m_click_coast_ctr == 0) {
                g_servo->enable(0.5f);
                m_servo360_brake_ctr = SERVO360_BRAKE_LOOPS;
        ...
        if (m_servo360_brake_ctr > 0) {
            // Braking phase: servo still active, but counting down
            m_servo360_brake_ctr--;
            if (m_servo360_brake_ctr == 0) {
                // Brake delay complete - full stop
                g_servo->disable();
                m_servo360_ctr      = 0;
                m_servo360_kick_ctr = 0;
            }
        } else if (m_servo360_ctr > 0) {
            // Normal alignment phase
            if (m_servo360_kick_ctr > 0) {
                m_servo360_kick_ctr--;
                if (m_servo360_kick_ctr == 0)
                    g_servo->enable(SERVO360_ALIGN_SPEED); // Kick vorbei → auf Zielgeschwindigkeit
            }
            m_servo360_ctr--;
            if (m_servo360_ctr == 0)
                g_servo->disable(); // 5s Fallback
        }
    }
    ...
    switch (m_state) {
        ...
        case STATE_REAL_APPROACH: {
            ...
            if (m_approach_ctr > ACCEL_LOOPS && wide_bar_active()) {
                ...
                // m_color_fallback intentionally NOT reset — holds driving-phase colour for early 360° nudge
                m_action_color     = 0;
                m_color_stable_ctr = 0;
                m_color_pending    = 0;
                m_arm_retract_ctr  = 0;
        ...
        case STATE_CROSSING_STOP:
            ...
            if (m_crossing_ctr == CROSSING_STOP_LOOPS - COLOR_READ_PHASE) {
                ...
                switch (m_action_color) {
                    ...
                    case 5:  g_servo_D1->enable(0.95f); break; // GRÜN: D1 ausfahren
                    case 7:  g_servo_D1->enable(0.95f); break; // BLAU: D1 ausfahren
                    default: break;
                }
            }
            // GRÜN: alle Balken direkt runter, gleiche Timing
            if (m_action_color == 5) {
                if (m_crossing_ctr == 60) {
                    if (m_crossings_left <= 2) { g_servo_D2->enable(SERVO_D2_PARTIAL_DOWN); }
                    else                        { g_servo_D2->enable(SERVO_D2_GRUEN_DOWN); }
                }
                if (m_crossing_ctr == 45) m_arm_retract_ctr = (m_crossings_left <= 2) ? 190 : 108;
            }
            // BLAU: gleiche Sequenz, andere D2-Tiefe
            if (m_action_color == 7) {
                if (m_crossing_ctr == 60) {
                    if (m_crossings_left <= 2) { g_servo_D2->enable(SERVO_D2_PARTIAL_DOWN); }
                    else                        { g_servo_D2->enable(SERVO_D2_BLAU_DOWN); }
                }
                if (m_crossing_ctr == 45) m_arm_retract_ctr = (m_crossings_left <= 2) ? 190 : 108;
            }
            // Schiebe/Zieh + Rückzug + 5-Click (Balken 1/2/3) oder 90°-Kick (Balken 4)
            if ((m_action_color == 5 || m_action_color == 7) && m_arm_retract_ctr > 0) {
                // Balken 2/3/4: Tablett um 5 Clicks rotiert → Jiggle-Richtung umkehren
                const bool rev = (m_crossings_left <= 2);
                if (rev && m_arm_retract_ctr == 155) g_servo->enable(SERVO360_CORRECT_SPEED); // zurück zu 45° (1s nach D2 start)
                if (rev && m_arm_retract_ctr == 152) g_servo->enable(0.5f);   // stop (500ms Pause → D2 voll)
                if (rev && m_arm_retract_ctr == 137) {                         // D2 voll runter — 500ms Pause vor Jiggle
                ...
                if (m_arm_retract_ctr == 108) g_servo->enable(rev ? 0.37f : 0.52f); // Jiggle Richtung 1
                if (m_arm_retract_ctr == 101) g_servo->enable(0.5f);          // stop — 7 Loops Richtung 1
                ...
                if (m_arm_retract_ctr == 52) g_servo->enable(rev ? 0.52f : 0.37f); // Jiggle Richtung 2
                if (m_arm_retract_ctr == 44) g_servo_D2->setPulseWidth(1.0f); // D2 hoch (360° läuft noch)
                if (m_arm_retract_ctr == 41) g_servo->enable(0.5f);           // stop — 11 Loops Richtung 2
                if (m_arm_retract_ctr == 22) {
                    g_servo_D1->setPulseWidth(0.0f);                           // D1 einfahren
                }
                if (m_arm_retract_ctr == 7) {
                    g_servo_D2->disable();                                     // D2 disable
                }
                m_arm_retract_ctr--;
                if (m_arm_retract_ctr == 0) {
                    g_servo_D1->disable();
                    if (m_crossings_left > 0) {
                        // Balken 1/2/3: 5 Clicks → Tablett für nächstes Päckchen weiterschalten
                        g_servo->enable(SERVO360_CLICK_SPEED);
                        m_click_target = 5;
                        m_click_cnt    = 0;
                    } else {
                        // Balken 4: 1 normale 90°-Drehung
                        g_servo->enable(SERVO360_KICK_SPEED);
                        m_click_target = 0;
                        m_click_cnt    = 0;
                    }
        ...
        case STATE_SMALL_CROSSING_STOP:
            ...
            if ((m_action_color == 5 || m_action_color == 7) && m_arm_retract_ctr > 0) {
                ...
                if (m_arm_retract_ctr == 14) {
                    g_servo_D2->disable();                                     // D2 disable (0.6s nach hoch)
                }
                m_arm_retract_ctr--;
                if (m_arm_retract_ctr == 0) {
                    g_servo_D1->disable();
        ...
        // ----------------------------------------------------------------
        // ROT_GELB_DRIVE: Linienfolge genau 120mm (encoder-basiert)
        // ----------------------------------------------------------------
        case STATE_ROT_GELB_DRIVE: {
            ...
            if (m_approach_ctr >= ROT_GELB_DRIVE_LOOPS) {
                g_M1->setVelocity(0.0f);
                g_M2->setVelocity(0.0f);
                m_arm_retract_ctr = (m_crossings_left <= 2 && !m_rot_gelb_is_small) ? 205 : 138;
        ...
        // ----------------------------------------------------------------
        // ROT_GELB_PAUSE: Arm-Sequenz (ROT→GRÜN-Logik, GELB→BLAU-Logik)
        // Balken 1+Schmallinien: 138 Loops; Balken 2/3/4: 205 Loops (25 Warten+3 Korr+25 Pause+29 Warten+108 Jiggle)
        // ----------------------------------------------------------------
        case STATE_ROT_GELB_PAUSE: {
            g_M1->setVelocity(0.0f);
            g_M2->setVelocity(0.0f);

            // Arm-Sequenz (ROT→GRÜN-Logik / GELB→BLAU-Logik)
            if (m_arm_retract_ctr > 0) {
                ...
                if (m_arm_retract_ctr == 205 &&  rev) g_servo_D1->enable(0.95f);
                if (m_arm_retract_ctr == 190 &&  rev) { g_servo_D2->enable(SERVO_D2_PARTIAL_DOWN); }
                if (m_arm_retract_ctr == 138 && !rev) g_servo_D1->enable(0.95f);
                if (m_arm_retract_ctr == 123 && !rev) g_servo_D2->enable(d2_depth);
                if (rev && m_arm_retract_ctr == 155) g_servo->enable(SERVO360_CORRECT_SPEED); // zurück zu 45° (1s nach D2 start)
                if (rev && m_arm_retract_ctr == 152) g_servo->enable(0.5f);   // stop (500ms Pause → D2 voll)
                if (rev && m_arm_retract_ctr == 137) { g_servo_D2->setPulseWidth(d2_depth); } // D2 voll runter — 500ms Pause
                if (m_arm_retract_ctr == 108) g_servo->enable(rev ? 0.37f : 0.52f); // Jiggle Richtung 1
                if (m_arm_retract_ctr == 101) g_servo->enable(0.5f);          // stop — 7 Loops Richtung 1
                ...
                if (m_arm_retract_ctr == 0) {
                    g_servo_D1->disable();
                    bool do_5click = (m_crossings_left > 0 && !m_rot_gelb_is_small);
                    if (do_5click) {
                        // Balken 1/2/3: 5 Clicks → Tablett für nächstes Päckchen weiterschalten
                        g_servo->enable(SERVO360_CLICK_SPEED);
                        m_click_target = 5;
                        ...
                        // Balken 4 + Schmallinien: 1 normale 90°-Drehung
                        g_servo->enable(SERVO360_KICK_SPEED);
                        m_click_target = 0;
                        m_click_cnt    = 0;
                    }
            ...
            // Exit: Arm + Drehung fertig → weiter
            if (m_arm_retract_ctr == 0 && m_servo360_ctr == 0) {
                m_color_delay_ctr = COLOR_READ_DELAY;
                if (m_rot_gelb_is_small) {
...
void roboter_v19_reset(DigitalOut& led)
{
    *g_en            = 0;
    g_M1->setVelocity(0.0f);
    g_M2->setVelocity(0.0f);
