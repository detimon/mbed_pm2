...
#ifdef CARGOSWEEP_FINAL_VERSION_03
...
static const int   D2_DOWN_DELAY_LOOPS = 23;  // ~0.45s warten bevor D2 runter fÃ¤hrt
...
static const float JIGGLE_AMPL_DEG    = 8.5f;  // +1.5Â°
...
static const float TRAY_JIGGLE_SPEED  = 0.20f; // muss > 0.14 sein (Totzone des Parallax-360)
...
{
    // Phase 0: D1 sofort raus, D2 erst nach D2_DOWN_DELAY_LOOPS (~0.25s) runter
    if (m_phase_idx == 0) {
        if (m_phase_ctr == 0) {
            g_servo_D1->enable(extensionForColor(m_action_color));
        }
        if (m_phase_ctr == D2_DOWN_DELAY_LOOPS) {
    ...
    // Phase 1: arm down + D1 jiggle (mehrere schnelle Zyklen statt einer langen)
    if (m_phase_idx == 1) {
        ...
        if (m_phase_ctr >= JIGGLE_LOOPS) {
            g_servo_D1->setPulseWidth(base);  // zurÃ¼ck zur Basis am Ende
            m_phase_idx = 2;
            m_phase_ctr = 0;
        }
        return false;
...
static void serviceTray()
{
    ...
    if (in_arm_state) {
        // Always update first â€” reads fresh angle AND runs P-controller.
        // Without this, getCurrentAngle() returns stale data and the window
        // calculation never triggers correctly.
        g_servoTray->moveToAngle(m_target_angle);
        g_servoTray->update();

        // V01-Logik (bewÃ¤hrt funktioniert): in_window bei nahem AnnÃ¤herungs-Endlauf
        // ODER bei kleinem ÃœberschieÃŸen (über die andere Seite via Wraparound).
        float ccw_dist = fmodf(g_servoTray->getCurrentAngle() - m_target_angle + 360.0f,
                               360.0f);
        bool in_window = (ccw_dist <= 35.0f) || (ccw_dist >= 185.0f);

        if (in_window) {
            // Jiggle nur in Phase 1 (Arm unten am Paket). Square-wave mit
            // Amplitude > Totzone (0.14) damit Tray sichtbar oszilliert.
            if (m_phase_idx == 1) {
            ...
            // Out of window: konstante Karussell-Drehung wie in V01.
            g_servoTray->stop();
            g_servoTray->addSpeed(-0.27f);
        }
    } else {
        // Navigation: update to keep angle fresh, then override with CCW rotation.
        g_servoTray->update();
        g_servoTray->stop();
        g_servoTray->addSpeed(-0.27f);
    }
...
{
    // Enable 360Â° servo immediately on button press
    ...
    *g_en = 1;
    ...
    switch (m_state) {
        ...
        // ----------------------------------------------------------------
        // SMALL_CROSSING_STOP â€” Dispatch-State (liest Farbe, geht zu STATE_SMALL_ARM_*)
        // ----------------------------------------------------------------
...
#endif // CARGOSWEEP_FINAL_VERSION_03
