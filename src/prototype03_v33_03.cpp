...
static const int   SF360_TIMEOUT_LOOPS = 100;
...
{
    switch (c) {
        case 3: return 0; // ROT   â†’ 0Â°
        case 5: return 1; // GRÃœN  â†’ 90Â°
        case 7: return 2; // BLAU  â†’ 180Â°
        case 4: return 3; // GELB  â†’ 270Â°
        default: return -1;
    }
}
...
// ---------------------------------------------------------------------------
// Ablege-Sequenz (runDeliverPhase) â€” wird jetzt ÃœBERALL verwendet
...
void roboter_v33_03_task(DigitalOut& led)
{
    ...
    *g_en = 1;
    ...
    switch (m_state) {
            ...
            if (m_approach_ctr > ACCEL_LOOPS && wide_bar_active()) {
                ...
                if (m_color_fallback != 0) {
                    ...
                    if (m_action_color == 5 || m_action_color == 7) {
                        m_phase_idx = 0; m_phase_ctr = 0; m_arm_retract_ctr = 1;
                    }
                }
        ...
        // ----------------------------------------------------------------
        // CROSSING_STOP â€” jetzt runDeliverPhase() statt runPickupPhase()
        // ----------------------------------------------------------------
        case STATE_CROSSING_STOP: {
            ...
            if (m_action_color == 0) {
                ...
                if (m_color_stable_ctr >= (m_color_pending == 5 || m_color_pending == 7 ? 1 : COLOR_STOP_STABLE_CNT)) {
            ...
            if (m_crossing_ctr == CROSSING_STOP_LOOPS - COLOR_READ_PHASE) {
                if (m_action_color == 0) {
                    m_action_color = m_color_fallback;
                    if (m_action_color == 0)
                        m_action_color = m_approach_fallback;
                    int slot = colorToSlot(m_action_color);
                    if (slot >= 0) {
                        m_target_angle = COLOR_ANGLE[slot];
                        trayMoveTo(m_target_angle);
        ...
        case STATE_SMALL_CROSSING_STOP: {
            ...
            if (m_small_crossing_ctr > SMALL_CROSSING_STOP_LOOPS - COLOR_READ_PHASE) {
                ...
                if (m_color_stable_ctr >= (m_color_pending == 5 || m_color_pending == 7 ? 1 : COLOR_STOP_STABLE_CNT) && m_action_color == 0) {
                    m_action_color = m_color_pending;
                    int slot = colorToSlot(m_action_color);
                    if (slot >= 0) {
                        m_target_angle = COLOR_ANGLE[slot];
        ...
        case STATE_ROT_GELB_DRIVE: {
            ...
            if (m_approach_ctr >= ROT_GELB_DRIVE_LOOPS) {
                g_M1->setVelocity(0.0f);
                g_M2->setVelocity(0.0f);
                m_action_color    = m_rot_gelb_color;
                m_phase_idx       = 0;
                m_phase_ctr       = 0;
                m_arm_retract_ctr = 1;
        ...
        // ----------------------------------------------------------------
        // ROT_GELB_PAUSE â€” jetzt runDeliverPhase() statt runPickupPhase()
        // ----------------------------------------------------------------
