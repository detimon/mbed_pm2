...
static const float D2_PARTIAL_DOWN    = 0.50f;   // Vorabsenkung vor Ausfahren
static const float D2_DOWN_BLAU       = 0.28f;
static const float D2_DOWN_GRUEN      = 0.07f;

// PICKUP / DELIVER timing (TEST_ARM_SEQUENCE: 1 s Pausen)
static const int PICKUP_PAUSE_LOOPS  = 50;   // 1 s — zwischen allen Pickup-Phasen
static const int JIGGLE_LOOPS        = 50;   // 1 s — ±10° Dreiphasen-Jiggle
static const int RAISE_LOOPS         = 50;   // 1 s — Arm hoch + einfahren
static const float JIGGLE_AMPL_DEG   = 10.0f; // validated in TEST_ARM_SEQUENCE
...
// Max wheel-speed differential while line-following.
// Allows ~200 mm radius curves (needs ~0.95 RPS for 250 mm radius at MAX_SPEED).
// Ghost reads produce up to 3.2 RPS diff → clamped to this value, no sharp turn.
static const float MAX_LINE_STEER = 1.2f;
...
// Same as applyLineFollowSpeed() but clamps the wheel speed differential.
// Prevents ghost-read spikes from causing sharp turns while still tracking curves.
static void applyLineFollowSpeedClamped(float scale)
{
    applyLineFollowSpeed(scale);
    float diff = m_cmd_M10 - m_cmd_M11;
    if (fabsf(diff) > MAX_LINE_STEER) {
        float avg  = (m_cmd_M10 + m_cmd_M11) * 0.5f;
        float half = (diff > 0.0f) ? MAX_LINE_STEER * 0.5f : -MAX_LINE_STEER * 0.5f;
        m_cmd_M10  = avg + half;
        m_cmd_M11  = avg - half;
        g_M10->setVelocity(m_cmd_M10);
        g_M11->setVelocity(m_cmd_M11);
...
void roboter_v29_init(int /*loops_per_second*/)
{
    ...
    g_servoHoriz->setMaxAcceleration(0.3f);  // D1 horizontal only — smooth in/out
...
// ---------------------------------------------------------------------------
// Pickup phase — sequence validated in TEST_ARM_SEQUENCE (2026-04-30).
// Phase 0 stellt sicher dass Drehteller am Zielwinkel ist bevor Arme fahren.
// Reihenfolge: Tray@target → D2 partial → D1 extend → D2 full → jiggle → D2 up → D1 retract.
// Returns true when complete.
// ---------------------------------------------------------------------------
static bool runPickupPhase()
{
    // Phase 0 — Drehteller auf m_target_angle bringen und warten.
    //   Garantiert korrekte Position vor jeder Arm-Bewegung, unabhängig davon
    //   ob CROSSING_STOP den Drehteller schon exakt positioniert hat.
    if (m_phase_idx == 0) {
        if (m_phase_ctr == 0)
            trayMoveTo(m_target_angle);
        m_phase_ctr++;
        // m_tray_moving wird von serviceTray() auf false gesetzt sobald isAtTarget().
        if (!m_tray_moving || m_phase_ctr >= SF360_TIMEOUT_LOOPS) {
            m_phase_idx = 1;
            m_phase_ctr = 0;
        }
        return false;
    ...
    // Phase 1 — D2 partial down (vor dem Ausfahren des horizontalen Arms).
    if (m_phase_idx == 1) {
        if (m_phase_ctr == 0) {
            g_servoHoriz->enable(D1_RETRACTED);
            g_servoVert ->enable(D2_PARTIAL_DOWN);
        }
        m_phase_ctr++;
        if (m_phase_ctr >= PICKUP_PAUSE_LOOPS) {
            m_phase_idx = 2;
            m_phase_ctr = 0;
        }
        return false;
    ...
    // Phase 2 — horizontalen Arm ausfahren.
    if (m_phase_idx == 2) {
        if (m_phase_ctr == 0)
            g_servoHoriz->setPulseWidth(extensionForColor(m_action_color));
        m_phase_ctr++;
        if (m_phase_ctr >= PICKUP_PAUSE_LOOPS) {
            m_phase_idx = 3;
            m_phase_ctr = 0;
    ...
    // Phase 4 — 2-D Jiggle: Tray ±10°, horizontal ±D1_JIGGLE_OFFSET.
    //   Dreiphasen analog TEST_ARM_SEQUENCE: -AMPL → +AMPL → zurück auf Ziel.
    if (m_phase_idx == 4) {
    ...
    // Phase 6 — horizontalen Arm einfahren, dann disable.
    if (m_phase_idx == 6) {
        if (m_phase_ctr == 0)
...
// ---------------------------------------------------------------------------
// Deliver phase — Tray zuerst auf Zielwinkel, dann Arm runter + jiggle + hoch.
// ---------------------------------------------------------------------------
static bool runDeliverPhase()
{
    ...
    // Phase 1 — extend horizontal, lower vertical to deliver depth.
    if (m_phase_idx == 1) {
        if (m_phase_ctr == 0) {
    ...
    // Phase 2 — 2-D jiggle: Tray ±10°, horizontal ±D1_JIGGLE_OFFSET.
    if (m_phase_idx == 2) {
        const float base = extensionForColor(m_action_color);
        const int   ph1  = JIGGLE_LOOPS / 3;
        const int   ph2  = 2 * JIGGLE_LOOPS / 3;
