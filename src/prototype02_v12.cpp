// CargoSweep — PROTOTYPE_02_V12
#include "test_config.h"

#ifdef PROTOTYPE_02_V12
...
static const float SERVO360_ALIGN_SPEED   = 0.509f; // CCW minimal — knapp über Totzone (0.5=Stopp)
...
// Public API
// ---------------------------------------------------------------------------
void roboter_v12_init(int loops_per_second)
{
...
void roboter_v12_task(DigitalOut& led)
{
    // --- Farbsensor ---
    ...
    switch (m_state) {
        ...
        case STATE_BLIND:
            ...
            if (all_sensors_active()) {
                ...
                g_servo->enable(SERVO360_ALIGN_SPEED);
        ...
        case STATE_CROSSING_STOP:
            ...
            if (m_action_color == 7) {
                if (m_crossing_ctr == 60) g_servo_D2->enable(0.8f);
                ...
                if (m_crossing_ctr == 25) {
                    g_servo_D1->disable();
                    g_servo->enable(SERVO360_ALIGN_SPEED); // drehen bis Endschalter
                    m_servo360_ctr = SERVO360_ALIGN_LOOPS;
                    s_endstop_hit  = false;
                }
            }
        ...
        case STATE_SMALL_CROSSING_STOP:
            ...
            // BLAU Sequenz: ausfahren → senken → hoch → einfahren → 360° bis Endschalter
            if (m_action_color == 7) {
                if (m_small_crossing_ctr == 60) g_servo_D2->enable(0.8f);
...
void roboter_v12_reset(DigitalOut& led)
{
    *g_en            = 0;
    g_M1->setVelocity(0.0f);
    g_M2->setVelocity(0.0f);
